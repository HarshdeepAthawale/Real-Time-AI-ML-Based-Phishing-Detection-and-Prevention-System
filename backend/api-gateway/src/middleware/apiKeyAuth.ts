...
import { connectPostgreSQL, getPostgreSQL } from '../../../shared/database/connection';
...
// Lazy database connection for API gateway
...
const ensureDatabaseConnection = async (): Promise<void> => {
  if (!dbInitialized) {
    try {
      ...
      dbInitialized = true;
      logger.info('Database connected for API key validation');
    } catch (error) {
      logger.error('Failed to connect to database for API key validation', { error });
      throw error;
    }
  }
};
