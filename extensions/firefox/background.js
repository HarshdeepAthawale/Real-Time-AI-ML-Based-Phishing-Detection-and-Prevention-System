...
// Note: Firefox MV2 does not support declarativeNetRequest; we use webNavigation only
...
function loadSettings() {
  browser.storage.local.get(['apiUrl', 'apiKey', 'blockThreats', 'blockSeverity']).then((result) => {
...
// Add threat to blocklist for future navigations (Firefox: no declarativeNetRequest)
function addThreatToBlocklist(url, threatInfo) {
  const norm = normalizeUrl(url);
  const ind = threatInfo.indicators;
  const indicators = Array.isArray(ind) ? ind : (ind ? [ind] : []);
  threatBlocklist.set(norm, {
...
// Remove from blocklist when user proceeds anyway
function removeThreatFromBlocklist(url) {
  const norm = normalizeUrl(url);
...
browser.webNavigation.onBeforeNavigate.addListener((details) => {
  ...
  if (threatInfo && shouldBlock(threatInfo.severity)) {
    ...
    const blockedUrl = browser.runtime.getURL(`blocked.html?${params.toString()}`);
...
async function checkURL(url, tabId) {
  ...
  try {
    ...
    if (isThreat) {
      await browser.storage.local.set({ [`threat_${tabId}`]: result });
      addThreatToBlocklist(url, result);
...
function redirectToBlockedPage(tabId, url, threatInfo) {
  ...
  const blockedPageUrl = browser.runtime.getURL(`blocked.html?${params.toString()}`);
  browser.tabs.update(tabId, { url: blockedPageUrl });
}

// Update badge based on threat status (Firefox MV2 uses browserAction)
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === 'getThreatInfo') {
    ...
    if (tabId) {
      browser.storage.local.get([`threat_${tabId}`]).then((result) => {
...
browser.alarms.create('cacheCleanup', { periodInMinutes: 30 });
